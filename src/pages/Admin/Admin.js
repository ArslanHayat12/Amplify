...
export const Admin = () => {
  ...
  const [isUserDeleteModalVisible, setIsUserDeleteModalVisible] =
    useState(false);
  const [isRoleDeleteModalVisible, setIsRoleDeleteModalVisible] =
    useState(false);
  const { dispatch, state } = useUserContext();
  const {
    dispatch: dispatchRole,
    state: { rolesList, currentRole },
  } = useRoleBasedContext();
  ...
  const handleEditModalForm = useCallback((user) => {
    dispatch({ type: "SELECTED_USER", payload: user });
    setIsUserModalVisible(true);
  }, []);

  const handleDeleteModalForm = useCallback((user) => {
    dispatch({ type: "SELECTED_USER", payload: user });
    setIsUserDeleteModalVisible(true);
  }, []);

  const handleRoleEditModalForm = useCallback((role) => {
    dispatchRole({ type: "SELECTED_ROLE", payload: role });
    setIsDashboardModalVisible(true);
  }, []);

  const handleRoleDeleteModalForm = useCallback((role) => {
    dispatchRole({ type: "SELECTED_ROLE", payload: role });
  ...
  const deleteUser = async () => {
    try {
      await API.del("userInfo", "/addUser/" + state.currentUser?.email);
      const apiData = await API.get("userInfo", "/addUser");
      dispatch({
        type: "SET_USERS_List",
        payload: apiData?.data?.Users || [],
  ...
  const DashboardFormModal = useMemo(() => {
    return (
      isDashboardModalVisible && (
        <DashboardModal
          isModalVisible={isDashboardModalVisible}
          setIsModalVisible={setIsDashboardModalVisible}
        />
      )
    );
  ...
  const MemoizedDeleteModal = useMemo(() => {
    return (
      <DeleteModal
        ...
        isModalVisible={isUserDeleteModalVisible}
        handleClose={() => setIsUserDeleteModalVisible(false)}
  ...
  const MemoizedRoleDeleteModal = useMemo(() => {
    return (
      <DeleteModal
        ...
        isModalVisible={isRoleDeleteModalVisible}
        handleClose={() => setIsRoleDeleteModalVisible(false)}
  ...
  return (
    <>
      ...
      <ButtonStyle>
        <Button onClick={() => handleDashboardModalForm()}>
